...
class NotificationService {
   ...
   */
  async create({ userId, type, title, body, metadata = {}, actionUrl = null, sendEmail = false, emailData = null }) {
    ...
    // Increment Redis unread counter (avoids COUNT query on badge render)
    ...
    if (sendEmail && emailData) {
      await addEmailJob('email:notification', {
        ...
        actionUrl,
        ...emailData,
      }).catch(err => logger.warn('Failed to queue notification email', { error: err.message }));
    }

    // Queue real-time push (Socket.io in Phase 9 will consume this)
    ...
    }).catch(() => {}); // non-critical — real-time is best-effort
  ...
  async createPriceDropNotification({
    userId, productId, productTitle, productSlug,
    ...
    if (alreadySent) {
      logger.debug('Price drop alert suppressed (dedup)', { userId, productId });
      return;
    }
    ...
    return this.create({
      ...
      title,
      body,
      metadata: { productId, productTitle, oldPrice, newPrice, dropAmount, dropPercent },
      actionUrl: `/products/slug/${productSlug}`,
      sendEmail: true,
      emailData: { productSlug, oldPrice, newPrice, dropAmount, dropPercent, productTitle },
    });
  }

  async getNotifications(userId, { cursor, limit, unreadOnly }) {
    return notificationRepository.findByUser(userId, { cursor, limit, unreadOnly });
  }
  ...
  async markAsRead(notificationId, userId) {
    ...
    if (result.modifiedCount > 0) {
      ...
      const current = parseInt(await redis.get(UNREAD_COUNT_KEY(userId)) || '0');
