...
class AnalyticsService {
  ...
  async getProductStats() {
    const [byCategory, byCondition, byStatus, topViewed, sellThroughRate] =
      await Promise.all([
        ...
        // Sell-through rate: sold / (sold + active) per category
        Product.aggregate([
          {
            $match: {
  ...
  async getAIPredictionStats() {
    // Compare predicted price vs actual sold price
    // Only for products that have both a prediction and a completed order
    const accuracy = await Order.aggregate([
      ...
      },
      {
        $addFields: {
          accuracyRate: {
            $multiply: [
              { $divide: ['$withinTenPct', '$sampleCount'] },
              100,
            ],
          },
        },
